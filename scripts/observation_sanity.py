...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Connect to a websocket feed and print live normalized observations.",
    )
    parser.add_argument(
        "exchange",
        nargs="?",
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Stop after printing this many observations. Zero means stream forever.",
    )
    parser.add_argument(
        "--log-level",
        default="INFO",
...
async def stream_observations(socket: BaseSocket, *, limit: int) -> None:
    count = 0
    try:
...
def main() -> None:
    ...
    asyncio.run(stream_observations(socket, limit=args.limit))
